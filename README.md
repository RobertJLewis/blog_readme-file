...
**Registered users** can create an account, log in, and access their personalised dashboard. They can add, edit, and delete posts, upload images to enhance their content, and engage with other users through comments and likes. Additional features include searching, filtering, and sorting posts to quickly find content, rating posts to track favourites, and managing their account details, such as updating their display name or profile information. Password recovery and optional social media login are also supported for convenience.  
...
8. Switch between local and external DB automatically Replace the temporary block in settings.py with the following conditional logic: if 'DATABASE_URL' in os.environ: 
DATABASES = { 'default': dj_database_url.parse(os.environ.get('DATABASE_URL')) } else: DATABASES = { 'default': { 'ENGINE': 'django.db.backends.sqlite3', 'NAME': BASE_DIR / 'db.sqlite3', } } 
...
## Media
  ...
  - **User-uploaded images** – Managed securely and efficiently through Cloudinary, which provides scalable cloud-based storage and delivery for profile pictures, post images, and other media.
